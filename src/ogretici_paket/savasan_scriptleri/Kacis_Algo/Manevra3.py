...
class ManevraNode:
    def __init__(self):
        ...
        self.initial_alt = 100
        self.target_list = [self.target_lat, self.target_lon]

        # 🎯 İkinci hedef konum (manevra sonrası gidilecek hedef)
        self.second_target_lat = -35.36089004
        self.second_target_lon = 149.16505423
        self.second_target_lat_srv = int(self.second_target_lat * 1e7)
        self.second_target_lon_srv = int(self.second_target_lon * 1e7)
        self.second_target_alt = 100
    ...
    def get_global_location(self, msg):
        ...
        # 6️⃣ İkinci hedefe yönel
        if self.returned and not self.sent_second_target:
            try:
                rospy.loginfo("İkinci hedefe yönlendiriliyor...")
                srv = rospy.ServiceProxy("/mavros/cmd/command_int", CommandInt)
                srv(broadcast=0, frame=6, command=192,
                    x=self.second_target_lat_srv,
                    y=self.second_target_lon_srv,
                    z=self.second_target_alt)
                rospy.loginfo("İkinci hedef komutu gönderildi.")
                self.sent_second_target = True
            except rospy.ServiceException:
                rospy.logwarn("İkinci hedefe yönlendirme sırasında hata.")
